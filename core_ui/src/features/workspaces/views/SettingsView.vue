...
<template>
	...
	<section class="summary-grid">
		<article class="surface-card">
			<UsersRound :size="19" />
			<div>
				<small>Identities</small><strong>{{ workspace.inventory.identities || 0 }}</strong>
			</div>
		</article>
		<article class="surface-card">
			<Link2 :size="19" />
	...
	<div class="boundary-note">
		...
		<div>
			...
			<span
				>Core owns the site experience. The separate Integration Hub owns Meta onboarding,
				template administration and durable delivery.</span
			>
		</div>
	</div>
	...
	<div class="settings-grid">
		<section class="surface-card settings-card">
			<header>
				...
				<Tag :value="`${workspace.channels.length} channels`" severity="info" rounded />
			</header>
